...
function generate_password() {
    ...
    while (password.length < length) {
        ...
        else if (random === 1 && uppercase_checkbox) {
            password += letters.charAt(Math.floor(Math.random() * letters.length)).toUpperCase();
        }
        else if (random === 2 && numbers_checkbox) {
            password += numbers.charAt(Math.floor(Math.random() * numbers.length));
        }
        else if (random === 3 && symbols_checkbox) {
...
function copy_password() {
    ...
    alert("Jakiś alert zrobić");
}

function toggle_password_visibility() {
    document.querySelector("#password").classList.toggle("blurred");
    let icon = document.querySelector("#password_visibility");
    let password_visibility = document.querySelector("#password").classList.contains("blurred");
    if (password_visibility) {
        icon.innerHTML = `<svg width="20px" height="20px" viewBox="0 0 28 28" fill="none" 
